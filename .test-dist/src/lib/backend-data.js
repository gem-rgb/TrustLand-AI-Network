...
function addToLedger(eventType, actorDid, eventData, targetDid = null, transactionId = null, agentId = null, accessTokenJti = 'system', authorizedScopes = ['system']) {
    ...
    t3VerifiableLedger.addEntry(eventType, actorDid, eventData, getPrivateKey(actorDid), publicKeyBase64, targetDid, transactionId, agentId, accessTokenJti, authorizedScopes);
    data.auditLogs.push({
        id: crypto.randomUUID(),
        action: eventType,
        actorDid,
...
export function initializeData() {
    ...
    institutions.forEach(inst => {
        ...
        const vcProof = signEd25519(hashData(JSON.stringify({ subjectDid: did, name: inst.name, role: inst.role, credentialType: inst.credentialType })), keyPair.privateKeyBase64);
        const vc = t3AgentAuthServer.issueVerifiableCredential(did, inst.name, inst.role || inst.credentialType, inst.org, inst.credentialType, 'did:t3:terminal3-issuer', vcProof);
        const identity = {
            did,
    ...
    agentDefs.forEach((def) => {
        ...
        const registration = t3AgentAuthServer.registerAgent(agentId, def.name, def.type, agentDid, scopes, agentKeyPair);
        const agent = {
            id: agentId,
...
export function getTrustScore(did) {
...
export function advanceWorkflow(workflowId, stepIndex) {
...
export function uploadDocument(docData) {
    ...
    const fileHash = hashData(docData.fileContent || docData.fileName);
    ...
    addToLedger('agent_action', docData.uploaderDid, { action: 'document_uploaded', documentType: docType, fileName: docData.fileName, verificationStatus: doc.verificationStatus, t3Authenticated: true });
    return doc;
}
export function sendMessage(msgData) {
    const senderDid = msgData.senderDid;
    const receiverDid = msgData.receiverDid;
    const msg = {
        id: crypto.randomUUID(),
        senderDid,
        receiverDid,
        ...
        relatedTransactionId: msgData.relatedTransactionId || null,
    ...
    addToLedger('agent_action', senderDid, { action: 'message_sent', messageType: msgData.messageType, subject: msgData.subject, t3Authenticated: true }, receiverDid, msgData.relatedTransactionId || null);
    return msg;
}
export function delegateAuthority(agentId, granterDid, permissionTypes) {
    ...
    const grant = t3AgentAuthServer.createPermissionGrant(granterDid, agent.identityDid, agentId, permissionTypes, grantSignature);
...
export function getPropertyVerifications(propertyId) {
    return data.propertyVerifications.filter(v => v.propertyId === propertyId);
}
export function getPropertyVerification(id) {
...
export async function generateDueDiligenceReport(params) {
    ...
    if (verifications.length === 0) {
        ...
        if (ownershipV)
            verifications.push(ownershipV);
        if (titleV)
            verifications.push(titleV);
        if (surveyV)
            verifications.push(surveyV);
    }
    ...
    const overallRiskLevel = overallRiskScore < 20 ? 'low' : overallRiskScore < 40 ? 'medium' : overallRiskScore < 70 ? 'high' : 'critical';
    // Generate summary
...
export function getDueDiligenceReports(propertyId) {
    return data.dueDiligenceReports.filter(r => r.propertyId === propertyId);
}
// ─── Trust Score Engine Functions ─────────────────────────────────────────────
export function calculateTrustScore(entityType, entityId) {
    ...
    if (entityType === 'user' || entityType === 'seller') {
        ...
        if (identity) {
            ...
            if (identity.verifiedAt) {
                score += 10;
                profile.scoringFactors.identityVerified = true;
            }
            // Account age bonus
            ...
            const completedTx = data.transactions.filter(t => (t.buyerDid === entityId || t.sellerDid === entityId) && t.status === 'completed').length;
        ...
        if (property) {
            ...
            if (ownershipAtts.length > 0) {
                score += 10;
                profile.scoringFactors.ownershipVerified = true;
            }
            // Survey verified
            ...
            if (surveyVerifications.length > 0) {
                score += 10;
            }
            ...
            if (legalVerifications.length > 0) {
                score += 10;
            }
        ...
        if (agent) {
            ...
            profile.scoringFactors.completedTransactions = data.transactions.filter(t => t.buyerAgentId === agent.id || t.sellerAgentId === agent.id).length;
        }
    }
    // Subtractions
    // -15 disputes
    const disputeCount = data.attestations.filter(a => (entityType === 'property' ? a.subjectDid === entityId : a.subjectDid === entityId)
        && a.attestationType === 'dispute').length;
    profile.disputes = disputeCount;
    score -= disputeCount * 15;
    // -20 fraud reports
    const fraudCount = data.riskReports.filter(r => r.riskLevel === 'critical' && r.findings.some(f => f.category === 'Fraud')).length;
...
export function getTrustProfile(entityId) {
...
export function updateTrustScoreOnEvent(eventType, entityId, entityType) {
    // Recalculate trust score after every verification and transaction event
    calculateTrustScore(entityType, entityId);
    // Add to audit ledger
    addAuditLedgerEntry(entityId, entityType === 'property' ? 'system' : entityType === 'seller' ? 'user' : entityType, 'trust_score_update', 'trust_profile', data.trustProfiles.find(p => p.entityId === entityId)?.id || entityId, { eventType, reason: 'Automatic trust score recalculation' });
}
...
function syncWorkflowForTransactionStage(transactionId, nextStage, actorId, notes) {
    ...
    if (workflow.context.autonomousPurchase === true && Array.isArray(workflow.steps)) {
...
export function advanceTransactionStage(transactionId, actorId, notes) {
    ...
    if (currentStageIndex === -1 || currentStageIndex >= TRANSACTION_STAGES.length - 1)
        return null;
...
export function getTransactionEvents(transactionId) {
    return data.transactionEvents.filter(e => e.transactionId === transactionId).sort((a, b) => new Date(a.timestamp).getTime() - new Date(b.timestamp).getTime());
}
export function getTransactionHistory(entityId) {
    return data.transactions.filter(t => t.buyerDid === entityId || t.sellerDid === entityId || t.propertyId === entityId);
}
// ─── Agent Marketplace Functions ──────────────────────────────────────────────
export function assignAgentToWorkflow(agentId, transactionId, role) {
    const agent = data.agents.find(a => a.id === agentId);
    if (!agent)
...
export function getAgentActivity(agentId) {
    ...
    if (!agent)
        return [];
    return data.transactionEvents.filter(e => e.actorId === agent.identityDid).sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime());
}
// ─── Immutable Audit Ledger Functions ─────────────────────────────────────────
export function addAuditLedgerEntry(actorId, actorType, action, resourceType, resourceId, metadata = {}) {
...
export function searchAuditLedger(filters) {
...
export function exportAuditLedger(format = 'json') {
    if (format === 'csv') {
        ...
        const rows = data.auditLedger.map(e => `${e.id},${e.actorId},${e.actorType},${e.action},${e.resourceType},${e.resourceId},${e.hash},${e.previousHash || ''},${e.timestamp}`).join('\n');
        return headers + rows;
    }
...
export function getAnalyticsMetrics(filters) {
    ...
    data.riskReports.forEach(r => { if (r.riskLevel in riskDistribution)
    ...
    transactions.forEach(t => { if (t.status in transactionPipeline)
        transactionPipeline[t.status]++; });
...
function generateVerificationFindings(verificationType, property, documents) {
    const findings = [];
...
function calculateVerificationRiskScore(findings) {
    if (findings.length === 0)
        return 50; // No findings = uncertain
    let score = 0;
    findings.forEach(f => {
        switch (f.severity) {
            case 'info':
        ...
        if (!f.verified)
            score += 15; // Unverified findings add risk
    });
    // Normalize to 0-100

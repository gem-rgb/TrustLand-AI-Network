...
const globalForPayments = globalThis;
...
const PAYMENT_PURPOSE_TARGET_STAGE = {
...
function paymentTransactionKey(transactionId, parcelId, purpose) {
...
function getPropertyOrThrow(parcelId) {
...
function getWorkflowTransactionOrNull(workflowTransactionId) {
    if (!workflowTransactionId)
        return null;
    return data.transactions.find((tx) => tx.id === workflowTransactionId) ?? null;
}
function isRoleAllowedForPurpose(role, purpose) {
    if (role === 'admin')
        return true;
    if (role === 'buyer') {
...
function isSessionAllowedForProperty(session, property) {
    if (session.role === 'admin')
        return true;
    if (session.role === 'seller')
        return property.ownerDid === session.userId;
    return true;
}
function parseSessionFromHeaders(headers) {
...
function rateLimitCreateIntent(session, parcelId) {
...
function deriveExpectedAmount(parcelId, purpose, workflowTransactionId) {
    const property = getPropertyOrThrow(parcelId);
    const basePrice = property.askingPrice;
    const paidReservations = Array.from(paymentStore.paymentsById.values()).filter((payment) => payment.parcelId === parcelId
...
function buildPaymentWorkflowSummary(record, propertyStatus, workflowTransactionStatus) {
    const directive = getPaymentWorkflowDirective(record.paymentPurpose);
    const paymentVerified = record.status === 'paid';
    const workflowStatus = record.workflowStatus || (paymentVerified
        ? propertyStatus === 'sold'
            ? 'settled'
...
function createAuditAndLedgerEntry(record, eventType, eventData, shouldPersistLedgerEntry = false) {
    ...
    if (!shouldPersistLedgerEntry)
        return null;
    const ledgerEntry = t3VerifiableLedger.addEntry(eventType, PAYMENT_SYSTEM_DID, {
        paymentId: record.id,
        transactionId: record.transactionId,
        parcelId: record.parcelId,
        payerUserId: record.payerUserId,
    ...
    }, paymentStore.paymentKeyPair.privateKeyBase64, paymentStore.paymentKeyPair.publicKeyBase64, record.payerUserId, record.transactionId, null, `payment_${record.id}`, ['payments:verify']);
    record.ledgerEntryId = ledgerEntry.id;
    return ledgerEntry;
}
function getPaymentInternalById(paymentId) {
    return paymentStore.paymentsById.get(paymentId) || null;
}
function getPaymentInternalByStripeIntentId(stripePaymentIntentId) {
...
function storePayment(record) {
    ...
    paymentStore.paymentsByTransactionPurposeKey.set(paymentTransactionKey(record.transactionId, record.parcelId, record.paymentPurpose), record.id);
    if (record.stripePaymentIntentId) {
        paymentStore.paymentsByStripeIntentId.set(record.stripePaymentIntentId, record.id);
    }
}
function getOrCreatePaymentRecord(session, input, amount, currency) {
...
function getWorkflowMutationStage(paymentPurpose) {
    return PAYMENT_PURPOSE_TARGET_STAGE[paymentPurpose] || null;
}
function maybeAdvanceWorkflowTransaction(record) {
...
function applyVerifiedPaymentEffects(record) {
...
function markPaymentState(record, status, details = {}) {
...
function recordLifecycleEvent(record, eventType, eventData, shouldAddLedgerEntry = false) {
    const ledgerEntry = createAuditAndLedgerEntry(record, eventType, eventData, shouldAddLedgerEntry);
    return ledgerEntry;
}
export function getPaymentSessionFromHeaders(headers) {
    return parseSessionFromHeaders(headers);
}
export function getPaymentRecord(paymentId, session) {
...
export function getPaymentStatusResponse(paymentId, session) {
    const record = getPaymentInternalById(paymentId);
    if (!record) {
...
export function getPaymentReceiptViewModel(paymentId, session) {
    ...
    const workflow = buildPaymentWorkflowSummary(payment, property.status, workflowTransaction?.status ?? null);
    return {
        payment,
        property,
        workflow,
...
export function listPayments(session) {
...
export function getPaymentDashboardStats() {
    ...
    const byPurpose = payments.reduce((acc, payment) => {
...
export function canSessionAccessPayment(session, payment) {
    if (session.role === 'admin')
        return true;
...
export async function createPaymentIntentRecord(input, session) {
    ...
    const { property, amount, currency } = deriveExpectedAmount(input.parcelId, input.paymentPurpose, input.workflowTransactionId);
...
export function getPaymentByTransactionKey(transactionId, parcelId, purpose) {
...
export function listPaymentsForTransactionReference(transactionReference) {
    return Array.from(paymentStore.paymentsById.values())
        .filter((payment) => payment.transactionId === transactionReference || payment.workflowTransactionId === transactionReference)
        .sort((a, b) => new Date(b.updatedAt).getTime() - new Date(a.updatedAt).getTime())
        .map(sanitizePaymentRecord);
}
export function markPaymentProcessing(paymentId, stripePaymentIntentId) {
    const payment = getPaymentInternalById(paymentId);
    if (!payment) {
        throw new TrustLandPaymentError('Payment not found', 404);
    }
...
export function markPaymentFailed(paymentId, failureReason, details = {}) {
    const payment = getPaymentInternalById(paymentId);
    if (!payment) {
        throw new TrustLandPaymentError('Payment not found', 404);
    }
...
export function markPaymentCancelled(paymentId, reason, details = {}) {
    const payment = getPaymentInternalById(paymentId);
    if (!payment) {
        throw new TrustLandPaymentError('Payment not found', 404);
    }
...
export function markPaymentRefunded(paymentId, details = {}) {
    const payment = getPaymentInternalById(paymentId);
    if (!payment) {
        throw new TrustLandPaymentError('Payment not found', 404);
    }
...
export function markPaymentDisputed(paymentId, details = {}) {
    const payment = getPaymentInternalById(paymentId);
    if (!payment) {
        throw new TrustLandPaymentError('Payment not found', 404);
    }
...
function resolveStripePaymentIntentIdFromEvent(event) {
    ...
    if (typeof payload.charge === 'string')
        return payload.charge;
    if (payload.charge && typeof payload.charge === 'object') {
        if (typeof payload.charge.payment_intent === 'string')
            return payload.charge.payment_intent;
        ...
        if (typeof payload.charge.id === 'string')
            return payload.charge.id;
    }
    if (typeof payload.id === 'string')
        return payload.id;
    return null;
}
async function resolveStripeChargePaymentIntentId(stripePaymentIntentId) {
    const stripe = getStripeServerClient();
    if (!stripe)
        return null;
    try {
...
export async function handleDemoPaymentConfirmation(paymentId, session) {
    const record = paymentStore.paymentsById.get(paymentId);
    if (!record) {
        throw new TrustLandPaymentError('Payment not found', 404);
    }
...
export function getPaymentRecordByStripeIntentId(stripePaymentIntentId) {
    const record = getPaymentInternalByStripeIntentId(stripePaymentIntentId);
    return record ? sanitizePaymentRecord(record) : null;
}
export function getPaymentRecordByPaymentTransaction(transactionId, parcelId, purpose) {
    const paymentId = paymentStore.paymentsByTransactionPurposeKey.get(paymentTransactionKey(transactionId, parcelId, purpose));
    if (!paymentId)
        return null;
    const record = paymentStore.paymentsById.get(paymentId);
...
export function getSafePaymentSummary(paymentId, session) {
    const record = getPaymentInternalById(paymentId);
    if (!record)
        return null;
    if (session && !canSessionAccessPayment(session, record)) {
...
export function isValidPaymentPurpose(value) {

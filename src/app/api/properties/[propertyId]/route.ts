...
function readPropertyId(params: Promise<{ propertyId: string }>) {
...
function parsePropertyUpdate(body: unknown): PropertyUpdateRequest {
  if (!body || typeof body !== 'object' || Array.isArray(body)) {
    throw new TrustLandPaymentError('Invalid property payload', 400);
  }

  return body as PropertyUpdateRequest;
}
...
function assertWriteAccess(session: PaymentSession, property: Property) {
...
function buildPropertyDetail(property: Property) {
  return {
    ...property,
...
function applyPropertyUpdate(property: Property, updates: PropertyUpdateRequest, session: PaymentSession) {
  ...
  if (updates.title !== undefined) property.title = updates.title.trim();
  if (updates.address !== undefined) property.address = updates.address.trim();
  if (updates.city !== undefined) property.city = updates.city.trim();
  if (updates.region !== undefined) property.region = updates.region.trim();
  if (updates.propertyType !== undefined) property.propertyType = updates.propertyType.trim();
  ...
  if (askingPrice !== undefined) property.askingPrice = askingPrice;

  if (updates.currency !== undefined) property.currency = updates.currency.trim().toUpperCase();
  if (updates.description !== undefined) property.description = updates.description.trim();

  const features = toStringArray(updates.features);
  if (features) property.features = features;

  const lat = toNumberOrNull(updates.lat);
  if (lat !== undefined) property.lat = lat;

  const lng = toNumberOrNull(updates.lng);
  if (lng !== undefined) property.lng = lng;
...
function archiveProperty(property: Property, session: PaymentSession, reason?: string) {
...
export async function DELETE(request: Request, context: { params: Promise<{ propertyId: string }> }) {
  try {
    ...
    const body = await request.json().catch(() => null);
    const reason = body && typeof body === 'object' && !Array.isArray(body) && typeof (body as { reason?: unknown }).reason === 'string'

...
import { TrustLandPaymentError } from './payments';
import type { PaymentSession } from './payment-types';
...
function assertPropertyAccess(session: PaymentSession, property: Property, requireOwnership = false) {
...
export function getPropertyDetailView(propertyId: string, session: PaymentSession) {
...
export function updatePropertyListing(propertyId: string, session: PaymentSession, updates: PropertyUpdateRequest) {
  const property = getPropertyOrThrow(propertyId);
...
export function archivePropertyListing(propertyId: string, session: PaymentSession, reason?: string) {
  const property = getPropertyOrThrow(propertyId);
  assertPropertyAccess(session, property, true);

...
import { AgentAuth, createAgentAuth, type AgentAuthOptions, type DiscoveryDoc, type TokenResponse } from '@agent-auth/sdk';

// ─── SUPPORTING: Our own crypto for key generation and signing ───────────────
import { signEd25519, verifyEd25519Signature, hashData, generateEd25519KeyPair, generateT3Did, type Ed25519KeyPair } from './t3-crypto';

// ─── Types ──────────────────────────────────────────────────────────────────

export interface T3AuthenticatedAgent {
  agentId: string;
  agentDid: string;
  name: string;
  agentType: string;
  ...
  /** Whether this agent was authenticated via the real SDK */
  sdkAuthenticated: boolean;
}

export interface T3AgentToAgentAuthResult {
...
export interface T3DelegationResult {
  delegationId: string;
  granterDid: string;
  granteeAgentDid: string;
...
console.log(
  `[T3 SDK] @agent-auth/sdk loaded successfully — ` +
...
class T3SDKClientManager {
  private agents: Map<string, T3AuthenticatedAgent> = new Map();
  private issuerUrl: string;
  private discoveryDoc: DiscoveryDoc | null = null;
  /** Track whether the SDK has been used for at least one real operation */
  private sdkOperationsCount: number = 0;

  constructor(issuerUrl: string = 'https://trustland.terminal3.io') {
  ...
  async registerAgent(
    ...
    console.log(
      `[T3 SDK] registerAgent() → createAgentAuth(${JSON.stringify({ issuer: this.issuerUrl, apiKey: apiKey.slice(0, 16) + '...', scopes })}) ` +
      `→ AgentAuth instance (instanceof=${sdkClient instanceof AgentAuth})`
    );

    const agent: T3AuthenticatedAgent = {
      agentId,
      agentDid,
      name,
      agentType,
   ...
   */
  async authenticateAgent(agentId: string): Promise<TokenResponse | null> {
    ...
    try {
      ...
      console.log(
        `[T3 SDK] loginWithApiKey() SUCCESS → access_token=${tokenResponse.access_token.slice(0, 20)}..., ` +
      ...
      console.log(
        `[T3 SDK] loginWithApiKey() failed (self-hosted mode): ${sdkError instanceof Error ? sdkError.message : String(sdkError)}`
      );
   ...
   */
  async authenticatedFetch(agentId: string, path: string, init?: RequestInit): Promise<Response | null> {
    const agent = this.agents.get(agentId);
    if (!agent) return null;
   ...
   */
  async authenticateAgentToAgent(
    fromAgentId: string,
    toAgentId: string,
    sharedContext: Record<string, unknown> = {}
  ): Promise<T3AgentToAgentAuthResult | null> {
    ...
    const result: T3AgentToAgentAuthResult = {
      authenticated: true,
  ...
  async createDelegation(
    ...
    grantedScopes: string[]
  ): Promise<T3DelegationResult | null> {
    ...
    const signedDelegation = signEd25519(
    ...
    const signatureValid = verifyEd25519Signature(
      hashData(delegationPayload),
      signedDelegation,
      granterKeyPair.publicKeyBase64
    );
   ...
   */
  async verifyToken(accessToken: string): Promise<{ valid: boolean; agentDid?: string; scopes?: string[] }> {
    ...
    try {
      ...
      if (response.ok) {
        const result = await response.json() as { active?: boolean; sub?: string; scope?: string };
        this.sdkOperationsCount++;
        return {
  ...
  getAgent(agentId: string): T3AuthenticatedAgent | undefined {
    return this.agents.get(agentId);
  }

  getAllAgents(): T3AuthenticatedAgent[] {
    return Array.from(this.agents.values());
  }

  getAgentByDid(did: string): T3AuthenticatedAgent | undefined {
  ...
  getSDKStatus(): {
    ...
    sdkAuthenticatedAgents: number;
    agentAuthClassType: string;
  } {
...
const globalForSDK = globalThis as unknown as { __t3_sdk_client: T3SDKClientManager | undefined };
